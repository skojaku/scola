...
class Scola():
	...
	def __init__(self, working_directory,disp=True):
		self.disp = disp
		self.working_directory = working_direcotry

	# Main routine	
	def run(self, C_samp, L, null_model = "config", lambda_list = np.logspace(-10, 0, 30)[::-1], disp=True, gamma = 0.5):	
	
		# Create working directory	
		if not os.path.exists(self.working_direcotry):
			os.mkdir(self.working_direcotry)
			if disp:
				print("Directory " , self.working_direcotry,  "is created.")
	
		# The tentative results are saved in tentative-results.pickle in the working directory
		file_tentative_results = '%s/tentative-results.pickle' % self.working_directory
		networks, Lams = self.load_results(file_tentative_results) 
	
		# Compute the null correlation matrices
		C_null, num_parameters_null = self._compute_null_model(C_samp, null_model)

		N = C.shape[0] 
		I = float(len(lambda_list))
		if disp:
			progress = 0
			sys.stdout.write("[%-20s] %d%% (0/%d)" % ('='*np.floor(progress/10).astype(int), progress, I))
		for i, lam in enumerate(lambda_list):
	
			if lam in Lams: # Skip if lam is scanned already
				continue
		
			# Construct a network	
			W = self.construct_network(C_samp, C_null, W, L, lam)
	
			# Compute the extended BIC	
			EBIC = self._calc_EBIC(W, C_samp, C_null, L, gamma, num_parameters_null)

			# Save results
			neworks+=[{'W':W, 'EBIC':EBIC, 'lam':lam}]
			
			# Save tentative results
			self.save_results(file_tentative_results, networks)
		
			# Disply the progress
			if disp:
				progress = np.floor(100*i/I).astype(int)
				sys.stdout.write('\r')
				sys.stdout.write("[%-20s] %d%% (%d/%d)" % ('='*np.floor(progress/10).astype(int), progress, i, I))
				sys.stdout.flush()
	...
	def _compute_null_model(self, C_samp, null_model, disp):
		...
		if os.exist(file_null_model):
			with open(file_null_model, 'rb') as f:
				res = pickle.dump(f)
				C_null = res["C_null"]
				null_parameters_null = res["null_parameters_null"]
		else:
			if null_model == "white-noise":	
				C_null = np.matrix(np.eye(C.shape[0]))
				num_parameters_null = 0
			elif null_model == "hqs":	
				C_null = np.mean(np.triu(C_samp,1)) * np.ones(C.shape)
	...
	def construct_network(self, C_samp, C_null, W, L, lam):
		N = C.shape[0]
	...
	def _maximisation_step(self, C_samp, C_null, W_base, lam):
		...
		while (t < maxIteration) & (t <= (t_best + maxLocalSearch+1)):
			...
			if (t % quality_assessment_interval) == 0:
				s  = self._score(W, C_samp, C_null, lam)
				if s > maxscore:
					W_best = W
					maxscore = s
					t_best = t
	...
	def _penalized_likelihood(self, W, C_samp, C_null, lam):
		return self.loglikelihood(W, C_samp, C_null)  - lam * np.sum(np.abs(W)) 
	...
	def _load_results(self, filename):
		...
		if os.path.exists(fileanme): # if tentative-results.pickle exists
